...
function Strategy(options, verify) {
	...
	options.scope = options.scope.length < 0 ? ['wallet:user:read'] : ['wallet:user:read'].push(options.scope);
...
Strategy.prototype.userProfile = function(accessToken, done) {
	...
	this._oauth2.get(this._userProfileURL, accessToken, function(err, body, res) {
		...
		if (err) {
			...
			if (json && json.code && json.reason) {
				return done(new CoinbaseAPIError(json.code, json.reason));
		...
		var profile = CoinbaseProfile.parse(json);
